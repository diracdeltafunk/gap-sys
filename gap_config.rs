...
pub fn wrapper_header(layout: &HeaderLayout) -> String {
    ...
    include_str!("wrapper.h.in").replace("@GAP_SYS_INCLUDES@", includes)
}
...
mod tests {
    ...
    #[test]
    fn renders_wrapper_header_for_installed_layout() {
        let header = wrapper_header(&HeaderLayout::IncludeSubdir);

        assert_eq!(
            header.matches("#include <gap/libgap-api.h>").count(),
            1,
            "the include placeholder must be substituted exactly once"
        );
        assert!(header.contains("#include <gap/gap_all.h>"));
        assert!(!header.contains("@GAP_SYS_INCLUDES@"));
        assert!(header.contains("SYSGAP_Initialize"));
    }

    #[test]
    fn renders_wrapper_header_for_source_layout() {
        let header = wrapper_header(&HeaderLayout::Direct);

        assert_eq!(
            header.matches("#include \"libgap-api.h\"").count(),
            1,
            "the include placeholder must be substituted exactly once"
        );
        assert!(header.contains("#include \"gap_all.h\""));
        assert!(!header.contains("<gap/libgap-api.h>"));
        assert!(!header.contains("@GAP_SYS_INCLUDES@"));
        assert!(header.contains("SYSGAP_Initialize"));
    }
}
